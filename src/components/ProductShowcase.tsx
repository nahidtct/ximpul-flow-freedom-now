  ...
  return <section className="py-24 bg-background fade-on-scroll">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {products.map((product, index) => <Card key={index} className="border border-border bg-background shadow-sm hover:shadow-lg transition-all duration-300 rounded-2xl overflow-hidden">
              <CardContent className="p-8">
                {/* Product Image */}
                <div className="flex justify-center mb-8 bg-muted/30 rounded-2xl p-8 h-80">
                  <img src={product.image} alt={`${product.name} ${product.edition}`} className="max-h-full w-auto object-contain" />
                </div>
